...
local getData = __orange__.GetPlayerData
local setData = __orange__.SetPlayerData
local getArmour = __orange__.GetPlayerArmour
local getHealth = __orange__.GetPlayerHealth
local getPing = __orange__.GetPlayerPing

local sync = {
	__index = function(t, k)
		return getData(t.id, k)
	end,
...
local indexhandlers = {
	...
	model = function(t) return t:getModel() end,
	pos = function(t) local x, y, z = t:getPosition() return { x = x,  y = y, z = z } end,
	health = function(t) return t:getHealth() end,
	armour = function(t) return t:getArmour() end,
	ping = function(t) return t:getPing() end,
	sync = function(t) return t._sync end
}

local newindexhandlers = {
	model = function(t, v) return t:setModel(v) end,
	health = function(t, v) return t:setHealth(v) end,
	armour = function(t, v) return t:setArmour(v) end,
}
...
function Player:new(id)
	...
	function obj:getArmour()
		return getArmour(self.id)
	end

	function obj:getDimension()
	...
	function obj:getPing()
		return getPing(self.id)
	end
	...
	function obj:getSyncedData(key)
		return getData(self.id, key)
	end
	...
	function obj:setDimension(world)
		setWorld(self.id, world)
	end
	...
	function obj:setSyncedData(key, val)
		setData(self.id, key, val)
	end
...
function Player:Trigger(event, ...)
	for k, player in pairs(Player.__pool) do
		local result = player:trigger(event, ...)
		if result == false then
			return false
		end
	end
